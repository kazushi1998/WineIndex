...
public class DeleteVineyard extends AsyncTask<VineyardEntity, Void, Void> {
    private AppDataBase dataBase;
    private OnAsyncEventListener callback;
    private Exception exception;

    public DeleteVineyard(Context context, OnAsyncEventListener callback) {
    ...
    protected Void doInBackground(VineyardEntity... vineyards) {
        try {
            for(VineyardEntity vineyard : vineyards) {
                dataBase.vineyardDao().delete(vineyard);
            }
        } catch (Exception e) {
            exception = e;
        }
